    ...
    <aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-8 transition-transform -translate-x-full bg-white border-r border-gray-200 sm:translate-x-0" aria-label="Sidebar">
        <div class="h-full px-4 pb-4 overflow-y-auto bg-white">
            <div class="grid gap-2 h-fit">
                ...
                <router-link v-if="role == 'super-admin'" :to="{name: 'AdminUsers'}" :class="active == 'users' ? 'text-white bg-custom-red' : ''" class="flex gap-4 align-middle font-medium rounded-lg py-2 px-2 hover:bg-custom-gray hover:bg-opacity-10 hover:text-custom-red"><AdjustmentsHorizontalIcon class="h-6 my-auto" />Users</router-link>
                ...
                <router-link :to="{name: 'AdminAgents'}" :class="active == 'agents' ? 'text-white bg-custom-red' : ''" class="flex gap-4 align-middle font-medium rounded-lg py-2 px-2 hover:bg-custom-gray hover:bg-opacity-10 hover:text-custom-red"><UserGroupIcon class="h-6 my-auto" />Agents</router-link>
                <router-link :to="{name: 'AdminDocuments'}" :class="active == 'documents' ? 'text-white bg-custom-red' : ''" class="flex gap-4 align-middle font-medium rounded-lg py-2 px-2 hover:bg-custom-gray hover:bg-opacity-10 hover:text-custom-red"><DocumentTextIcon class="h-6 my-auto" />Documents</router-link>
                <router-link :to="{name: 'AdminReview'}" :class="active == 'review' ? 'text-white bg-custom-red' : ''" class="flex gap-4 align-middle font-medium rounded-lg py-2 px-2 hover:bg-custom-gray hover:bg-opacity-10 hover:text-custom-red"><ClipboardDocumentCheckIcon class="h-6 my-auto" />Under Review</router-link> 
                <router-link :to="{name: 'AdminApproved'}" :class="active == 'approved' ? 'text-white bg-custom-red' : ''" class="flex gap-4 align-middle font-medium rounded-lg py-2 px-2 hover:bg-custom-gray hover:bg-opacity-10 hover:text-custom-red"><CheckBadgeIcon class="h-6 my-auto" />Approved</router-link>  
